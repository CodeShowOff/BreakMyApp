...
export default async function RunDetails(props: { params: Promise<{ id: string, runId: string }> }) {
  ...
  // Simulated state for demonstration of the progress UI
  const steps = [
  ...
  return (
    <div className="max-w-4xl">
      <header className="mb-8 border-b border-gray-200 dark:border-white/10 pb-4">
        <Link href={`/projects/${unwrappedParams.id}`} className="text-sm text-gray-500 hover:text-gray-900 dark:hover:text-gray-100 flex items-center gap-1 mb-4 transition-colors">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold tracking-tight text-gray-900 dark:text-gray-100 flex items-center gap-3">
              Test Run <span className="font-mono text-sm bg-gray-100 dark:bg-white/10 px-2 py-1 rounded">{unwrappedParams.runId}</span>
            </h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">Target: Invoicing App (Production)</p>
          </div>
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1.5 text-sm font-medium text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-3 py-1 rounded border border-blue-200 dark:border-blue-900/50">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="border border-gray-200 dark:border-white/10 rounded overflow-hidden bg-white dark:bg-[#0f0f0f]">
            ...
            <div className="p-4 font-mono text-xs bg-gray-50 dark:bg-black h-80 overflow-y-auto space-y-2 text-gray-600 dark:text-gray-400">
              ...
              <div className="text-amber-600 dark:text-amber-400">[00:01:50] Discovered potential IDOR on /api/invoices/v1 (Candidate finding generated)</div>
              <div className="flex items-center gap-2 mt-4 text-blue-500">
                <Loader2 className="h-3 w-3 animate-spin" /> <span>Executing tests (12/24)...</span>
              </div>
            </div>
          </div>
